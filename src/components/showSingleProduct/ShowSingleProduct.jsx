...
const ShowSingleProduct = ({ items }) => {
  ...
  const handleAddToCart = () => addToCart(productoFiltrado);
  
  const productoCarrito = getProductFiltrado(id)
  const btn = productoCarrito && productoCarrito.count >= productoCarrito.stock ? style.disabledBtn : ""
    return (
    <section className={style.Singelproduct_section}>
      <div className={style.product_continer}>
        ...
        <div className={style.product_info}>
          ...
          <button className={btn} onClick={ handleAddToCart}><img src={carrito} alt=""/>agregar al carrito</button>
